...
export async function subscribeToSignals(
  ...
  const source = String(formData.get("source") ?? "next-move").trim()
  ...
  if (webhookUrl) {
    ...
    return {
      status: "success",
      message: "You are in. We will send practical updates from Your Next Move when they are worth your attention.",
    }
  }
  ...
  try {
    ...
    return {
      ...
      message: alreadySubscribed
        ...
        : "You are in. We will send practical updates from Your Next Move when they are worth your attention.",
    }
  } catch {
    return {
      status: "error",
